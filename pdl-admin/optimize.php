  ...
  <tr>
    <td bgcolor=\"" . htmlspecialchars($template['table_border']) . "\">
      <table border=\"0\" cellpadding=\"3\" cellspacing=\"1\" width=\"100%\">
        <tr>
          <td bgcolor=\"" . htmlspecialchars($template['header_bg']) . "\" colspan=\"4\" align=\"center\">
            <b>Optimieren</b>
          </td>
        </tr>
        <tr>
          <td bgcolor=\"" . htmlspecialchars($template['header_bg']) . "\">
            <b>Tabelle</b>
          </td>
          <td bgcolor=\"" . htmlspecialchars($template['header_bg']) . "\">
            <b>Größe</b>
    ...
    while($opt_row = $db_handler->sql_fetch_array($opt_res))
     {
      ...
      $db_handler->sql_query("OPTIMIZE TABLE `" . $db_handler->sql_escape_string($opt_row['Name']) . "`");
      $alt = alt_switch();
      echo "
        <tr>
          <td bgcolor=\"" . htmlspecialchars($alt) . "\">
            " . htmlspecialchars($opt_row['Name']) . "
          </td>
          <td bgcolor=\"" . htmlspecialchars($alt) . "\">
    ...
    echo "
        <tr>
          <td bgcolor=\"" . htmlspecialchars($template['footer_bg']) . "\">
            <b>Gesamt: " . htmlspecialchars((string)$tabellen) . "</b>
          </td>
          <td bgcolor=\"" . htmlspecialchars($template['footer_bg']) . "\">
