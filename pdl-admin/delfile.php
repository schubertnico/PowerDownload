...
if($user_rights['editfiles'] == "Y")
 {
  if($submit == 1)
   {
    $release = $db_handler->sql_fetch_array($db_handler->sql_query("SELECT release_id FROM " . $sql_table['files'] . " WHERE file_id=" . sql_escape_int($file_id)));
    $release_id = $release['release_id'];
    $db_handler->sql_query("DELETE FROM " . $sql_table['files'] . " WHERE file_id=" . sql_escape_int($file_id));
    $db_handler->sql_query("DELETE FROM " . $sql_table['files'] . " WHERE mirror=" . sql_escape_int($file_id));
