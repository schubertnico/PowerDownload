...
if($user_rights['editfiles'] == "Y")
 {
  if($submit == 1)
   {
    $release_data = $db_handler->sql_fetch_array($db_handler->sql_query("SELECT * FROM " . $sql_table['screens'] . " WHERE screen_id=" . sql_escape_int($screen_id)));
    $db_handler->sql_query("DELETE FROM " . $sql_table['screens'] . " WHERE screen_id=" . sql_escape_int($screen_id));
